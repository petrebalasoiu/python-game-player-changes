...
if changes_at_a_time == '1':
    ...
    while len(players_removed) < MAX_CHANGES:
        try:
            removed_player1, removed_player2 = input('Choose two players to replace: \n').split()
        except ValueError:
            print('Only 2 valid names separated by a space are allowed. This program has stopped.')
            break
        try:
            added_player1, added_player2 = input('Choose two players from the bench: \n').split()
        except ValueError:
            print('Only 2 valid names separated by a space are allowed. This program has stopped.')
            break
